...
| Control keys exist | `npx wrangler d1 execute immi-catalog --remote --command "SELECT summary_key, value_int FROM catalog_summary WHERE summary_key LIKE 'rebuild_%'"` | `rebuild_generation`, `rebuild_applied_generation`, `rebuild_last_at`, `rebuild_last_attempt_at`, `rebuild_lease_until`, `rebuild_last_mutation_at`, `rebuild_dirty_since`, `rebuild_day`, `rebuild_count_today` once activity resumed (9 keys) |
