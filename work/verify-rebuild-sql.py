...
REPO = Path("/Volumes/Storm Breaker/Developer/IMMI-Case-")
statements = json.loads((REPO / "work/rebuild-statements.json").read_text())
...
check("rebuild stamps last rebuild time", abs(state.get("rebuild_last_at", 0) - int(time.time())) < 60, state)
...
check("mutation timestamp is refreshed", abs(last_mutation - int(time.time())) < 60, last_mutation)
